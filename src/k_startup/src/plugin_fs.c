...
plugin_t *new_plugin_fs(kernel_state_t *ks, file_sys_t *fs) {
    ...
    fernos_error_t put_err = FOS_E_UNKNWON_ERROR;
    if (nk_map && new_key_err == FOS_E_SUCCESS) {
        put_err = mp_put(nk_map, (void *)&root_nk, (const void *)&nk_entry);
    }

    if (!plg_fs || !nk_map || new_key_err != FOS_E_SUCCESS || !nk_entry || !bwq || put_err != FOS_E_SUCCESS) {
...
static fernos_error_t plg_fs_register_nk(plugin_fs_t *plg_fs, fs_node_key_t nk, fs_node_key_t *kernel_nk) {
    ...
    if (!nk) {
        return FOS_E_ABORT_SYSTEM; // If we use this incorrectly within the kernel, we wrote something
                                 // very wrong.
    }
    ...
    if (!nk_copy || !entry || !bwq || err != FOS_E_SUCCESS) {
...
static fernos_error_t plg_fs_cmd(plugin_t *plg, plugin_cmd_id_t cmd, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3) {
    ...
    if (err != FOS_E_SUCCESS) {
        DUAL_RET(thr, err, FOS_E_SUCCESS); // some sort of copy error?
    }
    path[path_len] = '\0';
    ...
    case PLG_FS_PCID_SET_WD: {
        ...
        if (err != FOS_E_SUCCESS || !(info.is_dir)) {
            fs_delete_key(plg_fs->fs, nk);
            err = !(info.is_dir) ? FOS_E_STATE_MISMATCH : err;
            DUAL_RET(thr, err, FOS_E_SUCCESS);
        }
    ...
    case PLG_FS_PCID_REMOVE: {
        ...
        if (mp_get(plg_fs->nk_map, &nk)) {
            ...
            fs_delete_key(plg_fs->fs, nk);
            DUAL_RET(thr, FOS_E_IN_USE, FOS_E_SUCCESS);
        }
    ...
    case PLG_FS_PCID_OPEN: {
        ...
        if (err != FOS_E_SUCCESS) {
            ...
            if (kernel_nk && plg_fs_deregister_nk(plg_fs, kernel_nk) != FOS_E_SUCCESS) {
                return FOS_E_ABORT_SYSTEM;
            }
...
static fernos_error_t plg_fs_on_fork_proc(plugin_t *plg, proc_id_t cpid) {
    ...
    if (!child) {
        return FOS_E_STATE_MISMATCH; // Something is very wrong if this is the case.
    }
...
static fernos_error_t copy_fs_handle_state(handle_state_t *hs, process_t *proc, handle_state_t **out) {
    ...
    if (!nk_entry || !*nk_entry) {
        return FOS_E_ABORT_SYSTEM; // Catastrophic error.
    }
    (*nk_entry)->references++;
 ...
 * If the given file handle's position is already SIZE_MAX, FOS_E_NO_SPACE will be returned
 * in the calling thread.
 *
 * The total number of bytes written is written to `*written`.
 * FOS_E_SUCCESS does NOT mean all bytes were written!!! (This will likely write in chunks to 
...
static fernos_error_t fs_hs_write(handle_state_t *hs, const void *u_src, size_t len, size_t *u_written) {
    ...
    if (fs_hs->pos == SIZE_MAX)  {
        DUAL_RET(thr, FOS_E_NO_SPACE, FOS_E_SUCCESS);
    }
    ...
    if (bytes_left < bytes_to_write) {
        ...
        if (err != FOS_E_SUCCESS) {
            return FOS_E_UNKNWON_ERROR; // We'll say this is a fatal error for now.
        }
 ...
 *
 * Just like with `sc_fs_write`, FOS_E_SUCCESS does NOT mean `len` bytes were read. 
