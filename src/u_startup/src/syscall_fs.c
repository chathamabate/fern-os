...
static fernos_error_t sc_fs_set_wd_p(const char *path, size_t path_len) {
    return trigger_syscall(SCID_FS_SET_WD, (uint32_t)path, path_len, 0, 0);
}

fernos_error_t sc_fs_set_wd(const char *path) {
    return sc_fs_set_wd_p(path, str_len(path));
}

static fernos_error_t sc_fs_touch_p(const char *path, size_t path_len) {
    return trigger_syscall(SCID_FS_TOUCH, (uint32_t)path, path_len, 0, 0);
}

fernos_error_t sc_fs_touch(const char *path) {
    return sc_fs_touch_p(path, str_len(path));
}

static fernos_error_t sc_fs_mkdir_p(const char *path, size_t path_len) {
    return trigger_syscall(SCID_FS_MKDIR, (uint32_t)path, path_len, 0, 0);
}

fernos_error_t sc_fs_mkdir(const char *path) {
    return sc_fs_mkdir_p(path, str_len(path));
}

static fernos_error_t sc_fs_remove_p(const char *path, size_t path_len) {
    return trigger_syscall(SCID_FS_REMOVE, (uint32_t)path, path_len, 0, 0);
}

fernos_error_t sc_fs_remove(const char *path) {
    return sc_fs_remove_p(path, str_len(path));
}

static fernos_error_t sc_fs_get_info_p(const char *path, size_t path_len, fs_node_info_t *info) {
...
fernos_error_t sc_fs_get_info(const char *path, fs_node_info_t *info) {
    return sc_fs_get_info_p(path, str_len(path), info);
}

static fernos_error_t sc_fs_get_child_name_p(const char *path, size_t path_len, size_t index, char *child_name) {
...
fernos_error_t sc_fs_get_child_name(const char *path, size_t index, char *child_name) {
    return sc_fs_get_child_name_p(path, str_len(path), index, child_name);
}

static fernos_error_t sc_fs_open_p(const char *path, size_t path_len, file_handle_t *fh) {
...
fernos_error_t sc_fs_open(const char *path, file_handle_t *fh) {
    return sc_fs_open_p(path, str_len(path), fh);
}
